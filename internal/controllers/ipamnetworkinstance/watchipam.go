...
func (e *EnqueueRequestForAllIpams) add(obj runtime.Object, queue adder) {
	...
	d := &ipamv1alpha1.IpamNetworkInstanceList{}
	if err := e.client.List(e.ctx, d); err != nil {
		return
	}

	for _, networkinstance := range d.Items {
		// only enqueue if the topology name match
		if networkinstance.GetIpamName() == dd.GetName() {
			queue.Add(reconcile.Request{NamespacedName: types.NamespacedName{
				Namespace: networkinstance.GetNamespace(),
				Name:      networkinstance.GetName()}})
		}
	}
}
